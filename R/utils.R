...
get_exported_dataset <- function(dataset_name) {
  ...
  expo_data_nms <- utils::data(package = "JRCENCRQCS")$results[, "Item"]
  if (!dataset_name %in% expo_data_nms) {
    stop(
      ...
      "maintainer: ", utils::maintainer("JRCENCRQCS")
    )
  }
...
get_internal_dataset <- function(dataset_name) {
  ...
  pkg_env <- as.environment("package:JRCENCRQCS")
  object_nms <- getNamespaceExports("JRCENCRQCS")
